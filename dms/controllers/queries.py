...
def item_character(doctype, txt, searchfield, start, page_len, filters):
	conditions = []

	return frappe.db.sql("""select 'tabitem character'.name, 'tabitem character'.item
		from 'tabitem character'
		where 	and 'tabitem character'.`{key}` like %(txt)s
			{fcond} {mcond}
		order by
			if(locate(%(_txt)s, name), locate(%(_txt)s, name), 99999),
			idx desc, name
